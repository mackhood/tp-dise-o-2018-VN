...
public class SistemaInteligente {
    ...
    public int cantidadDIencendidos(Cliente unCliente)
    {
        ...
        return unCliente.getDispositivosInteligentes().stream().filter(unDI -> unDI.estaEncendido()).collect(Collectors.toList()).size();
    }

    public int cantidadDIapagados(Cliente unCliente)
    {

        return unCliente.getDispositivosInteligentes().stream().filter(unDI -> unDI.estaApagado()).collect(Collectors.toList()).size();
    }
